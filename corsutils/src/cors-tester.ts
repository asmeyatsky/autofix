...
import { CorsConfig, CorsTestResult } from './types';

export class CorsTester {
  private async makeRequest(url: string, method: string = 'GET', headers: Record<string, string> = {}): Promise<AxiosResponse> {
    try {
      return await axios({
        method,
        url,
  ...
  private extractCorsHeaders(response: AxiosResponse): Record<string, string> {
  ...
  private getRequiredCorsHeaders(method: string = 'GET'): string[] {
  ...
  async testUrl(url: string, method: string = 'GET'): Promise<CorsTestResult> {
    try {
      // Test actual request
  ...
  private generateRecommendations(corsHeaders: Record<string, string>, missingHeaders: string[], method: string): string[] {
  ...
  async testMultipleUrls(urls: string[], methods: string[] = ['GET', 'POST']): Promise<CorsTestResult[]> {
  ...
  async testPreflight(url: string): Promise<CorsTestResult> {
    try {
