...
import { CorsTester } from './cors-tester';
import { CorsConfigGenerator } from './config-generator';
import { OutputFormatter } from './output-formatter';

export class CorsUtils {
  private corsTester: CorsTester;
  private configGenerator: CorsConfigGenerator;
  private outputFormatter: OutputFormatter;

  constructor() {
    this.corsTester = new CorsTester();
    this.configGenerator = new CorsConfigGenerator();
    this.outputFormatter = new OutputFormatter();
  }

  async testUrl(url: string, methods: string[] = ['GET', 'POST']) {
  ...
  async testMultipleUrls(urls: string[], methods: string[] = ['GET', 'POST']) {
  ...
  async testPreflight(url: string) {
  ...
  async analyzeCors(frontendUrl: string, backendUrl: string) {
  ...
  async generateConfig(serverType: string, origins: string[] = ['http://localhost:3000']) {
  ...
  async diagnoseCorsIssues(urls: string[]) {
    console.log(`🩺 Diagnosing CORS issues...`);
    
    const allResults: CorsTestResult[] = [];
  ...
  async runCorsTestSuite(urls: string[]) {
