...
exports.CorsConfigGenerator = void 0;
class CorsConfigGenerator {
    constructor() {
        this.serverTypes = [
            {
                name: 'Express.js',
    ...
    generateBasicCorsConfig(allowedOrigins = ['http://localhost:3000']) {
        return {
...
MIDDLEWARE = [
    ...
    generateConfig(serverType, config) {
...
// Access-Control-Allow-Credentials: ${config.credentials}`;
    ...
    analyzeCorsIssues(testResults, frontendUrl, backendUrl) {
        const issues = [];
        ...
        const suggestedConfig = this.generateBasicCorsConfig(frontendUrl ? [frontendUrl] : ['http://localhost:3000']);
        return {
            frontendUrl,
            backendUrl,
            issues,
