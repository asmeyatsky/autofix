...
export class OutputFormatter {
  ...
  printFullResults(results: LinkResult[], stats: CrawlerStats, duration: number): void {
    ...
    console.log('');
    console.log(chalk.blue.bold('📊 Link Results:'));
    console.log('');
    ...
    if (deadLinks.length > 0) {
      console.log(chalk.red.bold(`❌ Dead/Broken Links (${deadLinks.length}):`));
      deadLinks.forEach((link, index) => {
        console.log(chalk.red(`  ${index + 1}. ${link.url}`));
        console.log(chalk.red(`     Status: ${link.statusCode || 'Error'} ${link.statusText || link.error}`));
        console.log(chalk.red(`     Source: ${link.sourceUrl}`));
        if (link.linkText) {
          console.log(chalk.red(`     Text: "${link.linkText}"`));
        }
        if (link.responseTime) {
          console.log(chalk.red(`     Time: ${link.responseTime}ms`));
        }
        console.log('');
      });
    }
    ...
    if (redirectedLinks.length > 0) {
      console.log(chalk.yellow.bold(`🔀 Redirected Links (${redirectedLinks.length}):`));
      redirectedLinks.slice(0, 10).forEach((link, index) => {
        console.log(chalk.yellow(`  ${index + 1}. ${link.url}`));
        console.log(chalk.yellow(`     Status: ${link.statusCode} ${link.statusText}`));
        if (link.redirectChain?.length) {
          console.log(chalk.yellow(`     Chain: ${link.redirectChain.join(' → ')}`));
        }
      });
      if (redirectedLinks.length > 10) {
        console.log(chalk.yellow(`     ... and ${redirectedLinks.length - 10} more`));
      }
      console.log('');
    }
    ...
    if (validLinks.length > 0) {
      console.log(chalk.green.bold(`✅ Valid Links (${validLinks.length}):`));
      validLinks.slice(0, 5).forEach((link, index) => {
        console.log(chalk.green(`  ${index + 1}. ${link.url}`));
        console.log(chalk.green(`     Status: ${link.statusCode} ${link.statusText}`));
        if (link.contentType) {
          console.log(chalk.green(`     Type: ${link.contentType}`));
        }
      });
      if (validLinks.length > 5) {
        console.log(chalk.green(`     ... and ${validLinks.length - 5} more`));
      }
      console.log('');
    }
  }
  ...
  printSummary(stats: CrawlerStats, duration: number): void {
    console.log(chalk.blue.bold('📈 Crawling Summary:'));
    console.log(chalk.gray(`   Total pages crawled: ${stats.totalPages}`));
    console.log(chalk.gray(`   Total links checked: ${stats.checkedLinks}`));
    console.log(chalk.green(`   ✅ Valid links: ${stats.validLinks}`));
    console.log(chalk.red(`   ❌ Dead/broken links: ${stats.deadLinks}`));
    console.log(chalk.yellow(`   🔀 Redirected links: ${stats.redirectedLinks}`));
    console.log(chalk.red(`   🚨 Errors: ${stats.errors}`));
    console.log(chalk.gray(`   ⏱️  Duration: ${duration?.toFixed(2) || 'N/A'}s`));
    
    const errorRate = stats.checkedLinks > 0 ? parseFloat(((stats.deadLinks + stats.errors) / stats.checkedLinks * 100).toFixed(1)) : 0;
    if (errorRate > 10) {
      console.log(chalk.red(`   📉 Error rate: ${errorRate}% ⚠️`));
    } else if (errorRate > 5) {
      console.log(chalk.yellow(`   📉 Error rate: ${errorRate}%`));
    } else {
      console.log(chalk.green(`   📉 Error rate: ${errorRate}%`));
    }
  }

  printDeadLinks(deadLinks: LinkResult[], stats: CrawlerStats): void {
    console.log(chalk.red.bold(`💀 Dead/Broken Links Found: ${deadLinks.length}`));
    console.log('');
    ...
    console.log(chalk.red.bold(`📊 Summary: ${deadLinks.length}/${stats.checkedLinks} links are broken`));
  }

  async saveToFile(results: LinkResult[], stats: CrawlerStats, filePath: string, format: string): Promise<void> {
  ...
  private async saveAsJson(results: LinkResult[], stats: CrawlerStats, filePath: string): Promise<void> {
    const data = {
  ...
  private async saveAsCsv(results: LinkResult[], filePath: string): Promise<void> {
  ...
  private async saveAsHtml(results: LinkResult[], stats: CrawlerStats, filePath: string): Promise<void> {
    const html = this.generateHtmlReport(results, stats);
    await fs.writeFile(filePath, html, 'utf-8');
  }

  private async saveAsMarkdown(results: LinkResult[], stats: CrawlerStats, filePath: string): Promise<void> {
    const markdown = this.generateMarkdownReport(results, stats);
    await fs.writeFile(filePath, markdown, 'utf-8');
  }

  private generateHtmlReport(results: LinkResult[], stats: CrawlerStats): string {
    const deadLinks = results.filter(r => (r.statusCode && r.statusCode >= 400) || r.error);
    const validLinks = results.filter(r => r.statusCode && r.statusCode >= 200 && r.statusCode < 400);
    const redirectedLinks = results.filter(r => r.statusCode && r.statusCode >= 300 && r.statusCode < 400);
...
<body>
  ...
  private generateMarkdownReport(results: LinkResult[], stats: CrawlerStats): string {
    const deadLinks = results.filter(r => (r.statusCode && r.statusCode >= 400) || r.error);
    const validLinks = results.filter(r => r.statusCode && r.statusCode >= 200 && r.statusCode < 400);
    const redirectedLinks = results.filter(r => r.statusCode && r.statusCode >= 300 && r.statusCode < 400);
...
|---|-----|--------|--------|-----------|
${deadLinks.map((link, index) => 
  `| ${index + 1} | [${link.url}](${link.url}) | ${link.statusCode || 'Error'} ${link.statusText || link.error} | [${link.sourceUrl}](${link.sourceUrl}) | "${link.linkText || ''}" |`
).join('\n')}
` : ''}
...
|---|-----|--------|--------------|
${redirectedLinks.slice(0, 20).map((link, index) => 
  `| ${index + 1} | [${link.url}](${link.url}) | ${link.statusCode} ${link.statusText} | ${link.redirectChain?.join(' → ') || ''} |`
).join('\n')}
` : ''}
