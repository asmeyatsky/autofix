...
import { AutoFixConfig, DetectedError, MonitoringResult, ConsoleMessage, NetworkError } from '../types';
...
export class BrowserMonitor {
  ...
  async monitorFrontend(): Promise<MonitoringResult> {
    ...
    try {
      ...
      let pageLoadError: string | undefined;
  ...
  private async checkPageSuccess(page: Page): Promise<boolean> {
    try {
      ...
      const successChecks = await Promise.allSettled([
        ...
        page.evaluate(() => {
          ...
          return !loadingSelectors.some(selector => {
            const element = document.querySelector(selector);
            return element && (element as any).offsetParent !== null;
          });
        }),
        ...
        page.evaluate(() => {
          ...
          // Angular
          if ((window as any).ng) return true;
      ...
      const passedChecks = successChecks.filter(result => 
